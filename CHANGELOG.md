# Changelog — DataShredder v2

## v2.0.1 (2026-08-25)

### Bug Fixes

- **Canceling no longer deletes the file being shredded.** When cancellation arrived mid-file, the partially overwritten file was still deleted and counted as a success. The worker now stops before deletion, leaves the file in place, and reports it as canceled.
...
- **Progress bar can reach 100% when files are skipped.** Files skipped as read-only or symbolic links are now subtracted from the total byte estimate instead of leaving the bar stuck below 100%.
- **Progress updates throttled.** Progress was posted to the event dispatch thread once per 64 KB chunk; updates now post only when the percentage changes or 100 ms have passed since the last post.
